...
const FakeRSRP_1 = require("./sensors/FakeRSRP");
...
const sensors = (nmea, acc, temp, rsrp) => {
    ...
    if (rsrp) {
        sensors.set('rsrp', new FakeRSRP_1.default(rsrp, true, 5000));
    }
    return sensors;
};
function startSimulation({ config, nmea, acc, temp, rsrp }) {
    return __awaiter(this, void 0, void 0, function* () {
        const configuration = Configuration_1.readConfiguration(config);
        const app = new App_1.default(new PairingEngine_1.PairingEngine(pairingMethods), new AWSIoTHostConnection_1.AWSIoTHostConnection(configuration), sensors(nmea, acc, temp, rsrp));
        app.main();
    });
}
program
